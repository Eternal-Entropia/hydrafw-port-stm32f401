...
static const char* str_pins_i2c[] = {
	"SCL: PB6\r\nSDA: PB7\r\n",
	"SCL: PB10\r\nSDA: PB3\r\n",
	"SCL: PA8\r\nSDA: PB4\r\n",
};
static const char* str_prompt_i2c[] = {
	"i2c1" PROMPT,
	"i2c2" PROMPT,
	"i2c3" PROMPT,
};
	...
	for (t = token_pos; p->tokens[t]; t++) {
		...
		case T_DEVICE:
			...
			if (arg_int < 1 || arg_int > 3) {
				cprintf(con, "I2C device must be 1, 2 or 3.\r\n");
				return t;
			}
			init_proto_default(con);
			...
			cprintf(con, "Note: I2C parameters have been reset to default values.\r\n");
			break;
		case T_PULL:
			switch (p->tokens[++t]) {
			case T_UP:
...
static int show(t_hydra_console *con, t_tokenline_parsed *p)
{
	...
	if (p->tokens[1] == T_PINS) {
		tokens_used++;
		cprint(con, str_pins_i2c[proto->dev_num], strlen(str_pins_i2c[proto->dev_num]));
	} else {
		show_params(con);
	}
...
static void scan(t_hydra_console *con, t_tokenline_parsed *p)
{
	...
	for (i = 0x1; i < 0x78; i++) {
		bsp_i2c_start(proto->dev_num);
		bsp_i2c_master_write_u8(proto->dev_num, i << 1, &ack);
		bsp_i2c_stop(proto->dev_num);
		if (ack) {
...
{
	mode_config_proto_t* proto = &con->mode->proto;
	return str_prompt_i2c[proto->dev_num];
}
