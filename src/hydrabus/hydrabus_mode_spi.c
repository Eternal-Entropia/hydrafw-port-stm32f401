...
static const char* str_pins_spi3= {
	"CS:   PA4 (SW)\r\nSCK:  PA5\r\n"
};
...
const uint32_t spi_speeds[3][SPI_SPEED_NB] = {
	/* SPI1 (PA15, PB3..PB5 -> hardware SPI3 on APB1) */
	{
		160000,
		320000,
		650000,
	...
	/* SPI2 (PB12..PB15 -> hardware SPI2 on APB1) */
	{
		160000,
		320000,
		650000,
	...
	/* SPI3 (PA4..PA7 -> hardware SPI1 on APB2) */
	{
		320000,
		650000,
		1310000,
...
static int exec(t_hydra_console *con, t_tokenline_parsed *p, int token_pos)
{
	...
	for (t = token_pos; p->tokens[t]; t++) {
		...
		case T_DEVICE:
			...
			if (arg_int < 1 || arg_int > 3) {
				cprintf(con, "SPI device must be 1, 2 or 3.\r\n");
				return t;
			}
			init_proto_default(con);
...
static int show(t_hydra_console *con, t_tokenline_parsed *p)
{
	...
	if (p->tokens[1] == T_PINS) {
		...
		} else if (proto->dev_num == 1) {
			cprint(con, str_pins_spi2, strlen(str_pins_spi2));
			if (proto->config.spi.dev_bidir == 0) {
				cprint(con, str_pins_spi2_io, strlen(str_pins_spi2_io));
			} else {
				cprintf(con, "IO:   %s\r\n", proto->config.spi.dev_mode == DEV_MASTER ? "PB15" : "PB14");
			}
		} else {
			cprint(con, str_pins_spi3, strlen(str_pins_spi3));
			if (proto->config.spi.dev_bidir == 0) {
				cprint(con, str_pins_spi3_io, strlen(str_pins_spi3_io));
			} else {
				cprintf(con, "IO:   %s\r\n", proto->config.spi.dev_mode == DEV_MASTER ? "PA7" : "PA6");
			}
		}
	} else {
		show_params(con);
