...
int cmd_adc(t_hydra_console *con, t_tokenline_parsed *p)
{
	...
	if (p->tokens[1] == 0) {
		cprintf(con, "Usage: adc <adc1/adc2/adc3/tempsensor/vrefint/vbat> [period (nb ms)] [samples (nb sample)/continuous]\r\n");
		cprintf(con, "Examples:\r\n");
		cprintf(con, " adc adc1\r\n");
		cprintf(con, " adc adc1 period 100 samples 10\r\n");
		return TRUE;
	}
	...
	while (p->tokens[t]) {
		...
		case T_ADC1:
			...
			break;
		case T_ADC2:
			sources[num_sources++] = BSP_DEV_ADC2;
			break;
		case T_ADC3:
			sources[num_sources++] = BSP_DEV_ADC3;
			break;
		case T_TEMPSENSOR:
