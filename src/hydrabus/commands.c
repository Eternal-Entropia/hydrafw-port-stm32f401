...
const t_token_dict tl_dict[] = {
	...
	{ T_IDCODE, "idcode" },
	{ T_OOCD, "openocd" },
	/* Placeholder keeps dict indexes aligned with token enum.
	   The T_RNG token itself was removed from tl_tokens. */
	{ T_RNG, "random" },
	{ T_TWOWIRE, "2-wire" },
	{ T_CAN, "can" },
	{ T_ID, "id" },
...
/* STM32F401 has no CAN peripheral: compile CAN tokens out. */
#if defined(CAN1)
...
t_token tokens_sump[] = {
	{
		T_ARG_UINT,
		.help = "Channels count: 1, 2, 4, 8, 16"
	},
	{
		T_PINS,
		.arg_type = T_ARG_UINT,
		.help = "Channels count: 1, 2, 4, 8, 16 (default 16)"
	},
	{
		T_HELP,
		.arg_type = T_ARG_HELP,
		.help = "SUMP logic analyzer mode"
	},
	{ }
};
...
/* STM32F401 has no DAC peripheral: compile DAC tokens out. */
#if defined(DAC)
...
t_token tl_tokens[] = {
	...
	},
	{
		...
		.help_full = "Usage: adc <adc1/adc2/adc3/tempsensor/vrefint/vbat> [period (nb ms)] [samples (nb sample)/continuous]"
	},
#if defined(DAC)
	{
		T_DAC,
	...
	},
	{
		...
		.help = "SPI mode",
		.help_full = "Configuration: spi [device (1/2/3)] [pull (up/down/floating)] [mode (master/slave)] [frequency (value hz/khz/mhz)] [polarity 0/1] [phase 0/1] [msb-first/lsb-first]\r\nInteraction: [cs-on/cs-off] <read/write (value:repeat)> [exit]"
	},
	{
		T_I2C,
		.subtokens = tokens_i2c,
		.help = "I2C mode",
		.help_full = "Configuration: i2c [device (1/2/3)] [pull (up/down/floating)] [frequency (value hz/khz/mhz)]\r\nInteraction: [<start>] [<stop>] <read/write (value:repeat)>"
	},
	{
		T_ONEWIRE,
	...
	},
	{
		...
		.help = "UART mode",
		.help_full = "Configuration: uart [device (1/2/3)] [speed (value in bauds)] [parity (none/even/odd)] [stop-bits (1/2)]\r\nInteraction: <read/write (value:repeat)>"
	},
#ifdef HYDRANFC
	{
		T_NFC,
	...
	{
		T_SUMP,
		.subtokens = tokens_sump,
		.help = "SUMP mode (pins 1/2/4/8/16)"
	},
	{
		T_JTAG,
