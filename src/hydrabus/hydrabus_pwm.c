...
int cmd_pwm(t_hydra_console *con, t_tokenline_parsed *p)
{
	...
	if (p->tokens[1] == 0) {
		cprintf(con, "Usage: pwm <frequency (1Hz to 42MHz)> [duty-cycle (0 to 100%)] [exit]\r\n");
		cprintf(con, "Current frequency: %d Hz, Duty Cycle: %d%%\r\n", frequency, duty_cycle);
		cprintf(con, "Examples:\r\n");
		cprintf(con, " pwm frequency 1000 duty-cycle 50\r\n");
		return TRUE;
	}
