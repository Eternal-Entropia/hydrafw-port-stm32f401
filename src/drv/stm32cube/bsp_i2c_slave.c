...
{
	uint8_t value = 0;
	if(dev_num >= BSP_DEV_I2C_END) return 0;

	value = HAL_GPIO_ReadPin(bsp_i2c_hw[dev_num].sda_port, bsp_i2c_hw[dev_num].sda_pin) << 1;
	value |= HAL_GPIO_ReadPin(bsp_i2c_hw[dev_num].scl_port, bsp_i2c_hw[dev_num].scl_pin);
	return value;
}
