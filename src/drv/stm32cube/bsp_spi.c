...
static void spi_gpio_hw_init(bsp_dev_spi_t dev_num, uint32_t gpio_sck_miso_mosi_pull)
{
	...
	switch(dev_num) {
	case BSP_DEV_SPI1:
		/* Enable the SPI peripheral (SPI3 for PA15, PB3..PB5) */
		__SPI3_CLK_ENABLE();
	...
	case BSP_DEV_SPI3:
	default:
		/* Enable the SPI peripheral (SPI1 for PA4..PA7) */
		__SPI1_CLK_ENABLE();
		...
		GPIO_InitStructure.Pull  = GPIO_PULLUP;
		GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;
		GPIO_InitStructure.Pin = BSP_SPI3_NSS_PIN;
		HAL_GPIO_Init(BSP_SPI3_NSS_PORT, &GPIO_InitStructure);

		/* SPI SCK pin configuration */
		GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;
		GPIO_InitStructure.Pull  = gpio_sck_miso_mosi_pull;
		GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;
		GPIO_InitStructure.Alternate = BSP_SPI3_AF;
		GPIO_InitStructure.Pin = BSP_SPI3_SCK_PIN;
		HAL_GPIO_Init(BSP_SPI3_SCK_PORT, &GPIO_InitStructure);

		/* SPI MISO pin configuration */
		GPIO_InitStructure.Pin = BSP_SPI3_MISO_PIN;
		HAL_GPIO_Init(BSP_SPI3_MISO_PORT, &GPIO_InitStructure);

		/* SPI MOSI pin configuration */
		GPIO_InitStructure.Pin = BSP_SPI3_MOSI_PIN;
		HAL_GPIO_Init(BSP_SPI3_MOSI_PORT, &GPIO_InitStructure);
		break;
	}
...
static void spi_gpio_hw_deinit(bsp_dev_spi_t dev_num)
{
	...
	default:
		...
		/* Disable peripherals and GPIO Clocks */
		HAL_GPIO_DeInit(BSP_SPI3_NSS_PORT, BSP_SPI3_NSS_PIN);
		HAL_GPIO_DeInit(BSP_SPI3_SCK_PORT, BSP_SPI3_SCK_PIN);
		HAL_GPIO_DeInit(BSP_SPI3_MISO_PORT, BSP_SPI3_MISO_PIN);
		HAL_GPIO_DeInit(BSP_SPI3_MOSI_PORT, BSP_SPI3_MOSI_PIN);
		break;
	}
}
...
void bsp_spi_select(bsp_dev_spi_t dev_num)
{
	...
	case BSP_DEV_SPI3:
	default:
		HAL_GPIO_WritePin(BSP_SPI3_NSS_PORT, BSP_SPI3_NSS_PIN, GPIO_PIN_RESET);
		break;
	}
}
...
void bsp_spi_unselect(bsp_dev_spi_t dev_num)
{
	...
	case BSP_DEV_SPI3:
	default:
		HAL_GPIO_WritePin(BSP_SPI3_NSS_PORT, BSP_SPI3_NSS_PIN, GPIO_PIN_SET);
		break;
	}
}
...
uint8_t bsp_spi_get_cs(bsp_dev_spi_t dev_num)
{
	...
	case BSP_DEV_SPI3:
	default:
		return HAL_GPIO_ReadPin(BSP_SPI3_NSS_PORT, BSP_SPI3_NSS_PIN);
	}
}
