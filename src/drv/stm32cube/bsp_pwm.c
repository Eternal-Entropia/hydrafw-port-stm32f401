...
static void pwm_gpio_hw_init(bsp_dev_pwm_t dev_num)
{
	...
	if(dev_num == BSP_DEV_PWM1) {
		...
		gpio_init.Alternate = BSP_PWM1_AF;
 ...
 * \return __IO uint32_t*: TIM1->CCR1 to CCR4 register pointer
 *
 */
...
bsp_status_t bsp_pwm_init(bsp_dev_pwm_t dev_num)
{
	...
	/* Configure the PWM (TIM1) peripheral */
	__TIM1_CLK_ENABLE();
...
bsp_status_t bsp_pwm_update(bsp_dev_pwm_t dev_num, uint32_t frequency, uint32_t duty_cycle_percent)
{
	...
	/* TIM1 on APB2: PPRE2 = DIV1, timer clock = PCLK2 (no x2) */
	apb2_freq = bsp_get_apb2_freq();

	if(frequency < 1)
		return BSP_ERROR;

	if(frequency > apb2_freq)
		return BSP_ERROR;
	...
	uint32_t div = apb2_freq / frequency;
	uint32_t psc = 0;
	if (div > 65535) {
		psc = div / 65535;
		div = apb2_freq / ((psc + 1) * frequency);
	}
	arr = (div > 1) ? (div - 1) : 1;
