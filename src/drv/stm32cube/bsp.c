...
uint32_t bsp_get_apb2_freq(void)
{
	return STM32_PCLK2;
}
...
{
	/* Assert ULED (PC13, active low) as indicator we are in the bootloader */
	bsp_gpio_init(BSP_GPIO_PORTC, 13, MODE_CONFIG_DEV_GPIO_OUT_PUSHPULL, MODE_CONFIG_DEV_GPIO_NOPULL);
	bsp_gpio_clr(BSP_GPIO_PORTC, 13);
...
void bsp_enter_usb_dfu(void)
{
	...
	__GPIOA_CLK_ENABLE();
	/* Pulldown so that a floating PA0 never triggers DFU mode
	   (Black Pill has no user button wired to PA0). */
	bsp_gpio_init(BSP_GPIO_PORTA, 0, MODE_CONFIG_DEV_GPIO_IN, MODE_CONFIG_DEV_GPIO_PULLDOWN);
