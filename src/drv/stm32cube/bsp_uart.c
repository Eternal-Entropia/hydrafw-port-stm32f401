...
static void uart_gpio_hw_init(bsp_dev_uart_t dev_num)
{
	...
	} else if(dev_num == BSP_DEV_UART3) {
		/* Enable the UART peripheral */
		__USART6_CLK_ENABLE();

		GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;
		GPIO_InitStructure.Pull  = GPIO_NOPULL;
		GPIO_InitStructure.Speed = BSP_UART3_GPIO_SPEED;

		/* UART3 TX pin configuration */
		GPIO_InitStructure.Alternate = BSP_UART3_AF;
		GPIO_InitStructure.Pin = BSP_UART3_TX_PIN;
		HAL_GPIO_Init(BSP_UART3_TX_PORT, &GPIO_InitStructure);

		/* UART3 RX pin configuration */
		GPIO_InitStructure.Pin = BSP_UART3_RX_PIN;
		HAL_GPIO_Init(BSP_UART3_RX_PORT, &GPIO_InitStructure);
	}
}
...
static void uart_gpio_hw_deinit(bsp_dev_uart_t dev_num)
{
	if(dev_num == BSP_DEV_UART1) {
		...
		/* Disable peripherals GPIO */
		HAL_GPIO_DeInit(BSP_UART3_TX_PORT, BSP_UART3_TX_PIN);
		HAL_GPIO_DeInit(BSP_UART3_RX_PORT, BSP_UART3_RX_PIN);
	}
}
