	...
	for (w = 0; w < num_words; w++) {
		...
		if (done) {
			...
			case T_ARG_FLOAT:
				...
				if (*suffix) {
					if (!strcasecmp(suffix, "k") || !strcasecmp(suffix, "khz")) {
						arg_float *= 1000.0f;
					} else if (!strcasecmp(suffix, "m") || !strcasecmp(suffix, "mhz")) {
						arg_float *= 1000000.0f;
					} else if (!strcasecmp(suffix, "g") || !strcasecmp(suffix, "ghz")) {
						arg_float *= 1000000000.0f;
					} else if (!strcasecmp(suffix, "hz")) {
						/* no multiplier */
					} else {
						if (!complete_tokens)
							tl->print(tl->user, "Invalid value."NL);
